// ─── tui — live dashboard with session metrics ──────────────────────────────
//
...
fn draw_stats(frame: &mut Frame, area: Rect, state: &common::SessionState, stats: &analytics::anomaly::ProjectStats) {
    let items: Vec<Line> = vec![
        Line::from(vec![
            Span::styled("Turn: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", state.turn), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
            Span::styled("Files edited: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", state.files_edited.len()), Style::default().fg(Color::Green)),
        ]),
        Line::from(vec![
            Span::styled("Files read: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", state.files_read.len()), Style::default().fg(Color::Blue)),
        ]),
        Line::from(vec![
            Span::styled("Errors: ", Style::default().fg(Color::Gray)),
            Span::styled(
                format!("{}", state.errors_unresolved),
                Style::default().fg(if state.errors_unresolved > 0 { Color::Red } else { Color::Green }),
            ),
        ]),
        Line::from(vec![
            Span::styled("Explore count: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", state.explore_count), Style::default().fg(Color::Yellow)),
        ]),
        Line::from(vec![
            Span::styled("Tokens in: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}K", state.estimated_tokens_in / 1000), Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            Span::styled("Tokens saved: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}K", state.estimated_tokens_saved / 1000), Style::default().fg(Color::Cyan)),
        ]),
        Line::from(""),
        Line::from(vec![
            Span::styled("Project avg quality: ", Style::default().fg(Color::Gray)),
            Span::styled(
                if stats.quality_score.n >= 3 { format!("{:.0}/100", stats.quality_score.mean) } else { "N/A".to_string() },
        ...
        Line::from(vec![
            ...
            Span::styled(
                if stats.session_length.n >= 3 { format!("{:.0}", stats.session_length.mean) } else { "N/A".to_string() },
...
fn draw_charts(frame: &mut Frame, area: Rect, state: &common::SessionState) {
    ...
    let token_data: Vec<u64> = state.turn_snapshots.iter()
        .map(|s| (s.tokens_in_delta + s.tokens_out_delta) / 1000) // in K
        .collect();

    let sparkline = Sparkline::default()
        .block(Block::default().title(" Token Burn (K/turn) ").borders(Borders::ALL))
        .data(&token_data)
        .style(Style::default().fg(Color::Cyan));
    frame.render_widget(sparkline, chart_chunks[0]);

    // Phase timeline
    let transitions: Vec<Line> = state.adaptive.transitions.iter().map(|t| {
        ...
        Line::from(vec![
            Span::styled(format!("T{}: ", t.turn), Style::default().fg(Color::DarkGray)),
            Span::styled(&t.from, Style::default().fg(Color::Gray)),
            Span::raw(" → "),
            Span::styled(&t.to, Style::default().fg(color).add_modifier(Modifier::BOLD)),
            Span::styled(format!(" ({})", t.reason), Style::default().fg(Color::DarkGray)),
        ])
    }).collect();

    let phase_text = if transitions.is_empty() {
        vec![Line::from(Span::styled("No phase transitions yet", Style::default().fg(Color::DarkGray)))]
    } else {
        transitions
    };

    let phase_widget = Paragraph::new(phase_text)
        .block(Block::default().title(" Phase Timeline ").borders(Borders::ALL));
