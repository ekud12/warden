...
fn error_loop_detected_after_repeated_failures() {
    ...
    fire_hook("userprompt-context", &user_prompt("fix the build"), &cwd);
...
fn compaction_forecast_emits_event() {
    ...
    for i in 0..10 {
        ...
        fire_hook(
            "posttool-session",
            &posttool_bash("cat large_file.rs", &"x".repeat(2000), 0),
            &cwd,
        );
    }
    ...
    let any_tokens = state["turn_snapshots"]
        ...
        .map(|arr| {
            arr.iter()
                .any(|s| s["tokens_out_delta"].as_u64().unwrap_or(0) > 0)
        })
        .unwrap_or(false);
    assert!(
    ...
    for i in 0..8 {
        ...
        fire_hook(
            ...
            &posttool_bash(
                "cargo build",
                &format!(
                    "error[E0{}]: compilation failed in module_{}",
                    308 + i % 5,
                    i
                ),
                1,
...
fn dream_effectiveness_scores_persist() {
    ...
    std::fs::write(
        &eff_path,
        serde_json::to_string_pretty(&scores_json).unwrap(),
    )
    .unwrap();
