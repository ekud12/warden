...
fn rules_schema_is_valid_json() {
    ...
    assert!(
        parsed.is_ok(),
        "rules.schema.json is not valid JSON: {:?}",
        parsed.err()
    );
}
...
fn config_schema_covers_known_sections() {
    ...
    assert!(
        props.contains_key("assistant"),
        "Missing 'assistant' section"
    );
    assert!(
        props.contains_key("telemetry"),
        "Missing 'telemetry' section"
    );
    assert!(props.contains_key("tools"), "Missing 'tools' section");
    assert!(
        props.contains_key("restrictions"),
        "Missing 'restrictions' section"
    );
}
