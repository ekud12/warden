// ─── session_end — session lifecycle logging ────────────────────────────────
//
...
pub fn run(raw: &str) {
    ...
    {
        let project_dir = common::project_dir();
        let mut eff = analytics::effectiveness::load(&project_dir);
        analytics::effectiveness::update_session_end(&mut eff, &state.rules_fired, quality_score);
        analytics::effectiveness::save(&project_dir, &eff);
    }
