...
- **Compaction forecast** — linear regression on token usage predicts when context will compress
...
- **Anomaly detection** — Welford's online mean/variance, z-score flagging (>2σ)
...
- **Harbor bridges** — LangChain, CrewAI, AutoGen adapter stubs + BridgeConfig schema
