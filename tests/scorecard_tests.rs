...
fn trace_healthy_has_milestones() {
    ...
    assert!(
        milestones.len() >= 2,
        "healthy session should have 2+ milestones"
    );
}
...
fn trace_adversarial_all_bypasses_denied() {
    ...
    assert!(
        denials >= 8,
        "adversarial trace should have 8+ denied bypass attempts"
    );
}
...
fn trace_policy_heavy_has_substitution_denials() {
    ...
    assert!(
        denials.iter().any(|d| d.contains("grep")),
        "should deny grep"
    );
    assert!(
        denials.iter().any(|d| d.contains("find")),
        "should deny find"
    );
    assert!(
        denials.iter().any(|d| d.contains("chmod")),
        "should deny chmod"
    );
}
...
fn trace_read_heavy_has_advisory() {
    ...
    assert!(
        advisories >= 1,
        "read-heavy session should trigger advisory"
    );
}
...
fn trace_compaction_has_recovery() {
    ...
    let compactions: usize = trace
        .iter()
        .filter_map(|e| serde_json::from_slice::<serde_json::Value>(e).ok())
        .filter(|e| e.get("type").and_then(|v| v.as_str()) == Some("compaction"))
        .count();
    let post_compaction_milestones: usize = trace
        .iter()
        .filter_map(|e| serde_json::from_slice::<serde_json::Value>(e).ok())
        .filter(|e| {
    ...
    assert!(
        post_compaction_milestones >= 1,
        "should recover after compaction"
    );
}
...
fn trace_long_session_has_multiple_milestones() {
    ...
    assert!(
        milestones >= 5,
        "long session should have 5+ milestones, got {}",
        milestones
    );
}
...
fn trace_multi_subsystem_has_focus_advisory() {
    ...
    let focus_advisories: usize = trace
        .iter()
        .filter_map(|e| serde_json::from_slice::<serde_json::Value>(e).ok())
        .filter(|e| {
    ...
    assert!(
        focus_advisories >= 1,
        "multi-subsystem session should have focus advisory"
    );
}
...
fn trace_compression_heavy_has_compression_events() {
    ...
    assert!(
        compressions >= 3,
        "compression-heavy session should have 3+ compression events"
    );
}
